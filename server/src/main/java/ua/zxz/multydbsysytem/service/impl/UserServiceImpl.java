...
public class UserServiceImpl implements UserService {
    ...
    public UserDto getByUsername(String username) {
        return userMapper.entityToDto(userRepository.findByUsername(username)
                .orElseThrow(() -> new ResponseStatusException(NOT_FOUND, "User not found by username = [" + username + "]")));
    }
    ...
    public void saveUser(UserDto userDto) {
        if (existsByUsername(userDto.getUsername())) {
            ...
            throw new ResponseStatusException(BAD_REQUEST, "The user with such username already exists");
        } else if (existsByEmail(userDto.getEmail())) {
            log.info("The user with email = [{}] already exists", userDto.getEmail());
            throw new ResponseStatusException(BAD_REQUEST, "The user with such email already exists");
        }
    ...
    public void updateUser(UserDto userDto) {
        if (Objects.isNull(userDto.getId())) {
            ...
            throw new ResponseStatusException(BAD_REQUEST, "The user id is null");
        } else if (!existsByUsername(userDto.getUsername())) {
            log.info("The user with username = [{}] does not exist", userDto.getUsername());
            throw new ResponseStatusException(BAD_REQUEST, "The user with such username does not exist");
        } else if (!existsByEmail(userDto.getEmail())) {
            log.info("The user with email = [{}] does not exist", userDto.getEmail());
            throw new ResponseStatusException(BAD_REQUEST, "The user with such email does not exist");
        }
        userRepository.save(userMapper.dtoToEntity(userDto));
    }
